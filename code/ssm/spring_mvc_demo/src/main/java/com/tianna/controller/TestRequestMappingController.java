 ...
 * 作用：通过请求的请求方式匹配请求
 * method属性是RequestMethod类型的数组，即当前浏览器所发送请求的请求方式匹配method属性中的任何一种类型，
 * 则当前请求就会被注解所标识的方法进行处理
 * 若浏览器所发送的请求的请求路径和@RequestMapping注解value属性匹配但是请求方式不匹配，
 ...
 * ?:任意的单个字符（不包括?）
 * *:任意个数的任意字符（不包括?和/）
 * **:任意层数的任意目录，注意使用方式只能将**卸载双斜线中，前后不能右其他字符
 *
...
public class TestRequestMappingController {
    ...
    @RequestMapping(
            ...
            params = {"username","!paaword","age=20","gener!=女"}
            //headers = {"referer"}
    )
    public String hello(){
        return "success";
