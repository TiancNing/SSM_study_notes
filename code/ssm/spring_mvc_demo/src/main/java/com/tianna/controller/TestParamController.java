 ...
 * 可以获取form表单参数，也可以获取请求地址?username=1234&password=456的参数。
 *
 * 2、通过控制器方法的形参获取
 ...
 * required:设置是否必须传输value所对应的请求参数，默认值为true，表示value所对应的请求参数必须传输，否则页面报错 400 - Required String parameter 'userName' is not present
 * 若设置为false，则表示value所对应的请求参数不是必须传输，若不传输，则形参值为null。
 ...
 * 需要在控制器方法的形参位置设置实体类类型的形参，要保证实体类中的属性的属性名（不是变量名，是get、set方法去掉get、set后首字母小写）和请求参数的名字一致
 * 可以通过实体类类型的形参获取请求参数。
 *
 *
 *
